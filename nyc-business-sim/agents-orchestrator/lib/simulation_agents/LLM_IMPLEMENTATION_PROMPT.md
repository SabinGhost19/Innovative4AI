...
1. Înlocuiește `{AGENT_NAME}` cu numele agentului (ex: `market-context-agent`, `supplier-agent`)
...
Implementează agentul **{AGENT_NAME}** pentru sistemul NYC Business Simulator, conform arhitecturii complete definite în documentația de mai jos.
...
## 🎯 SPECIFICAȚII PENTRU AGENTUL {AGENT_NAME}
...
1. **Fișierul complet**: `{AGENT_NAME}.ts`
...
- Specificațiile pentru {AGENT_NAME}
- Checklist-ul de mai sus

Generează codul complet, funcțional, production-ready pentru **{AGENT_NAME}**.
